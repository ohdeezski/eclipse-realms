...
func _load_item_icon(item_id: String) -> Texture2D:
    # Load the icon texture for an item ID. Icons live in assets/ui/icons/.
    # game_data icon IDs are like 'items/iron_sword' -> basename 'iron_sword'.
    var base = item_id.split("/")[-1]
    # Primary: real icon sheet files generated by assemble_art / Mio masters
    var icon_path = "res://assets/ui/icons/%s.png" % base
    if ResourceLoader.exists(icon_path):
        return load(icon_path)
    # Legacy fallback path (kept for compatibility)
    var legacy = "res://resources/icons/%s.png" % item_id
